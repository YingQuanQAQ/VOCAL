...
def get_config(args, eval=False):
    ...
    config.commit_loss_k = 0.25
    config.lang_loss_k = 1.0
    config.anchor_loss_k = 1.0
    config.anchor_loss_min = 0.05
    config.slot_grouping = 'task'
    config.ae_pg = 0
    config.ae_type = ''  # ['', 'fc', 'mlp', 'rfc', 'rmlp']
    config.img_feat_dim = 64
    config.vqvib_num_protos = 16
    config.vqvib_beta = 0.05
    config.vqvib_commit_alpha = 0.25
    config.vqvib_entropy_weight = 0.02
    config.vqvib_recons_weight = 1.0
    ...
    if config.ablation != 'full':
        id_args += [['abl', config.ablation]]
...
def apply_ablation(config):
    if config.ablation == 'full':
        return

    if config.method != 'vocal':
        raise ValueError('Ablations are only supported for method=vocal')

    if config.ablation == 'no-slot':
        config.slot_grouping = 'single'
    elif config.ablation == 'no-commit':
        config.commit_loss_k = 0.0
    elif config.ablation == 'no-lang':
        config.lang_loss_k = 0.0
    elif config.ablation == 'no-anchor':
        config.anchor_loss_k = 0.0
        config.anchor_loss_min = 0.0
    else:
        raise ValueError('Unknown ablation: {}'.format(config.ablation))
