...
class Evaluator(mp.Process):
    ...
    def __init__(self, master, net, env, save_dir_fmt, gpu_id=0,
                 ...
                 num_eval_episodes=10, net_type='', method='vocal'):
        super().__init__()
        self.master = master
        self.net = net
    ...
    def get_major_events(self, info):
        events = []
        if info.get('red_door_opened_now', False):
            events.append('red_door_opened')
        if info.get('blue_door_opened_now', False):
            events.append('blue_door_opened')
    ...
    def decode_tokens(self, token_indices):
        ...
        for slot_idx, token_idx in enumerate(token_indices):
            if slot_idx < len(self.vocab) and token_idx < len(self.vocab[slot_idx]):
                words.append(self.vocab[slot_idx][token_idx])
        return ' | '.join(words)
        ...
        while not self.master.is_done():
            ...
            for eval_id in range(self.num_eval_episodes):
                ...
                while not check_done(done):
                    speaker_tokens = None
                    if self.net_type == 'ae':
                        plogit, _, hidden_state, comm_out, aux_info = self.net(
                            state_var, hidden_state, env_mask_idx=env_mask_idx)
                        _, _, ent, action = self.net.take_action(plogit,
                                                                 comm_out)
                        if self.method == 'vocal' and 'k_idx' in aux_info:
                            speaker_tokens = aux_info['k_idx'].cpu().tolist()
                    else:
                    ...
                    if self.method == 'vocal' and eval_id == 0 and \
                            speaker_tokens is not None:
                        major_events = self.get_major_events(info)
                        for agent_id, token_idx in enumerate(speaker_tokens):
                            print('[Step {}] Agent {} says: [{}]'.format(
                                env_copy.step_count,
                                agent_id,
                                self.decode_tokens(token_idx)))
                        if major_events:
                            print('[Step {}] Events: {}'.format(
                                env_copy.step_count,
                                ', '.join(major_events)))
