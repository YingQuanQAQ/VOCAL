...
def to_state_var(x, use_gpu=False, dtype=np.float32):
    if type(x) is dict:
        # multi-agent
...
def _to_state_var(x, use_gpu=False, dtype=np.float32):
...
def dict_to_state_var(x, use_gpu=False, dtype=np.float32):
    for k, v in x.items():
        v = np.array(v, dtype=dtype)
        var = torch.from_numpy(v)
        if k == 'pov':
            var = var.permute(2, 0, 1).unsqueeze(0)
        x[k] = var if use_gpu else var
    return x


def img_to_state_var(x, use_gpu=False, dtype=np.float32):
    x = np.array(x, dtype=dtype)
    var = torch.from_numpy(x)
    var = var.permute(2, 0, 1).unsqueeze(0)
    return var if use_gpu else var


def to_torch(x, use_gpu=False, dtype=np.float32):
    x = np.array(x, dtype=dtype)
    var = torch.from_numpy(x)
    return var if use_gpu else var
